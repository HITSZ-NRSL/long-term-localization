...
namespace {
bool is_lidar_point_with_ring = true;
}
...
void KittiReader::initialize(const QString& directory) {
  ...
  const int size_of_point = is_lidar_point_with_ring ? 5 : 4;
  for (uint32_t i = 0; i < velodyne_filenames_.size(); ++i) {
    ...
    if (!labels_dir.exists(filename)) {
      ...
      uint32_t num_points = in.tellg() / (size_of_point * sizeof(float));
      in.close();
...
void KittiReader::readPoints(const std::string& filename, Laserscan& scan) {
  ...
  for (uint32_t i = 0; i < num_points; ++i) {
    points[i].x = values[size_of_point * i];
    points[i].y = values[size_of_point * i + 1];
    points[i].z = values[size_of_point * i + 2];
    remissions[i] = values[size_of_point * i + 3];
  }
}
