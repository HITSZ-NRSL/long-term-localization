...
public:
  ...
  template <typename Graph>
  static std::vector<typename boost::graph_traits<Graph>::vertex_descriptor>
  findMaximumClique(const Graph &graph, const size_t min_clique_size) {
    ...
    for (size_t i = 0u; i < sorted_vertices.size(); ++i) {
      ...
      if (vertex_degree >= max_found_size) {
        ...
        // Get the size of the maximum clique contained in the subgraph defined by the current
      ...
      typename GraphTraits::out_edge_iterator e_it, e_end;
      for (boost::tie(e_it, e_end) = boost::out_edges(sorted_vertices[i], graph); e_it != e_end;
           ++e_it) {
        const Vertex neighbor = boost::target(*e_it, graph);
...
private:
  ...
  // access.
  template <typename Graph> static void assertIsUndirectedAndRandomAccessGraph(const Graph &graph) {
    BOOST_CONCEPT_ASSERT((boost::concepts::GraphConcept<Graph>));
    typedef boost::graph_traits<Graph> GraphTraits;
    typedef typename GraphTraits::vertex_descriptor Vertex;
    static_assert(
        std::is_same<typename GraphTraits::directed_category, boost::undirected_tag>::value,
  ...
  static size_t binSortVerticesByDegree(
    ...
    for (const auto degree : vertex_degrees)
      ++bin_sizes[degree];
  ...
  static size_t findMaximumCliqueSubset(
      ...
      const std::vector<size_t> &vertex_degrees, const size_t clique_size, size_t max_found_size,
    ...
    while (!subset.empty()) {
      ...
      if (clique_size + subset.size() <= max_found_size)
        break;
      ...
      // and its neighbors.
      const size_t new_found_size = findMaximumCliqueSubset(
          graph, neighbors, vertex_degrees, clique_size + 1u, max_found_size, maximum_clique_tmp);
