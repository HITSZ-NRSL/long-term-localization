...
void GraphBasedGeometricConsistencyRecognizer::recognize(const PairwiseMatches &predicted_matches) {
  ...
  std::vector<size_t> maximum_clique =
      GraphUtilities::findMaximumClique(consistency_graph, params_.min_cluster_size);
  // BENCHMARK_STOP("SM.Worker.Recognition.FindClique");

  if (maximum_clique.empty())
    return;
...
inline Eigen::Matrix4f GraphBasedGeometricConsistencyRecognizer::estimateRigidTransformation(
  ...
  const unsigned int n_matches_to_consider =
      std::min(static_cast<unsigned>(true_matches.size()), 8u);
