...
void GeometricConsistencyRecognizer::recognize(const PairwiseMatches &predicted_matches) {
  ...
  Cluster::PointCloud::Ptr first_cloud(new Cluster::PointCloud());
  Cluster::PointCloud::Ptr second_cloud(new Cluster::PointCloud());
  ...
  for (size_t i = 0u; i < predicted_matches.size(); ++i) {
    // First centroid.
    Cluster::PointT first_centroid = predicted_matches.at(i).centroids().first;
    first_cloud->push_back(first_centroid);
    // Second centroid.
    Cluster::PointT second_centroid = predicted_matches.at(i).centroids().second;
  ...
  std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>
      correspondence_transformations;

  typedef std::vector<pcl::Correspondences> Correspondences;
  Correspondences clustered_corrs;
  pcl::GeometricConsistencyGrouping<Cluster::PointT, Cluster::PointT>
  ...
  if (!clustered_corrs.empty()) {
    ...
    std::transform(ordered_indices.begin(), ordered_indices.end(),
                   candidate_transfomations_.begin(),
                   [&](const size_t i) { return correspondence_transformations[i]; });
