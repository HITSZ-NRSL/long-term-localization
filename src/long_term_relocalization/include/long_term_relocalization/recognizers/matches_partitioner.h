...
template <typename PartitionData> class MatchesGridPartitioning {
public:
...
public:
  ...
  static MatchesGridPartitioning<PartitionData>
  computeGridPartitioning(const PairwiseMatches &matches, float partition_size);
}; // class MatchesPartitioner
...
template <typename PartitionData>
MatchesGridPartitioning<PartitionData>
MatchesPartitioner::computeGridPartitioning(const PairwiseMatches &matches,
                                            const float partition_size) {

  // Validate inputs.
  CHECK_GT(partition_size, 0.0f);
  if (matches.empty())
    return MatchesGridPartitioning<PartitionData>();
