...
KeyframeUpdater::KeyframeUpdater(double keyframe_delta_trans,
    ...
    : keyframe_delta_translation_(keyframe_delta_trans),
      keyframe_delta_rotation_angle_(math::DegToRad(keyframe_delta_angle)),
      is_initialized_(true), accumulate_distance_(0.0),
      prev_keypose_(kindr::minimal::QuatTransformation::Identity()) {}

bool KeyframeUpdater::UpdatePose(const kindr::minimal::QuatTransformation &pose) {
  ...
  const kindr::minimal::QuatTransformation delta_pose = prev_keypose_.inverse() * pose;
