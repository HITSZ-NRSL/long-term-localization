...
IncrementalGeometricConsistencyRecognizer::IncrementalGeometricConsistencyRecognizer(
    ...
    : GraphBasedGeometricConsistencyRecognizer(params),
      ...
      max_consistency_distance_for_caching_(params.max_consistency_distance_for_caching +
                                            params.resolution),
      half_max_consistency_distance_for_caching_(params.max_consistency_distance_for_caching *
                                                 0.5f) {}
  ...
  for (const auto &cached_match_locations : cached_matches_locations) {
    ...
    for (const size_t candidate_cache_slot_index : match_cache.candidate_consistent_matches) {
      ...
      if (match_2_index != kNoMatchIndex_) {
        ...
        float consistency_distance =
            computeConsistencyDistance(match, match_2, max_consistency_distance_);
        ...
        if (consistency_distance <= max_consistency_distance_) {
          ...
          if (consistency_distance <= params_.resolution)
            boost::add_edge(match_2_index, cached_match_locations.match_index, consistency_graph);
        }
      }
    }
...
inline void IncrementalGeometricConsistencyRecognizer::processNewMatches(
    const PairwiseMatches &predicted_matches, const std::vector<size_t> &free_cache_slot_indices,
  ...
  for (size_t i = 0; i < partitioning.getHeight(); ++i) {
    for (size_t j = 0; j < partitioning.getWidth(); ++j) {
      for (const auto match_index : partitioning(i, j).match_indices) {
        ...
        if (match_index_to_cache_slot_index[match_index] != kNoCacheSlotIndex_)
          continue;
...
bool IncrementalGeometricConsistencyRecognizer::mustRemoveFromCache(const PairwiseMatch &match,
                                                                    const size_t cache_slot_index) {
...
IncrementalGeometricConsistencyRecognizer::buildConsistencyGraph(
  ...
  std::vector<size_t> match_index_to_cache_slot_index(predicted_matches.size(), kNoCacheSlotIndex_);
  ...
  processCachedMatches(predicted_matches, cached_matches_locations, cache_slot_index_to_match_index,
